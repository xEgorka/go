	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "positive test #4" {
				r.AddCookie(&http.Cookie{Name: "token", Value: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjIwMzM3MjQ1MzUsIlVzZXJJRCI6ImVlQHJyLnJ1In0.4aFlz_lViChwCqj6_eVv1j9AGU0Y1NVgdyOoO0ChGhs"})
			}
			h.ServeHTTP(NewCustomResponseWriter(), r)
		})
	}
